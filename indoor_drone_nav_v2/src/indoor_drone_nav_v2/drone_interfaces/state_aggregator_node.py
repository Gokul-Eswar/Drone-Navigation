...
class StateAggregatorNode(Node):
    """
    A ROS2 node that subscribes to various MAVROS topics, aggregates the information
    into a single custom DroneState message, and publishes it for other nodes to use.
    """
    def __init__(self):
        ...
        # --- Subscribers to raw MAVROS topics ---
        self.create_subscription(MavrosState, '/mavros/state', self._mavros_state_callback, 10)
        self.create_subscription(PoseStamped, '/mavros/local_position/pose', self._pose_callback, 10)
        ...
        self.get_logger().info("Subscribers to MAVROS topics created.")
    ...
    def _pose_callback(self, msg: PoseStamped):
        """Update pose and header information."""
        self._drone_state.pose = msg.pose
